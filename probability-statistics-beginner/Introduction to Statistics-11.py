...
mean_car_speed = statistics.mean(car_speeds)
...
degrees_zero = [float(i)+459.67 for i in fahrenheit_degrees]
...
s = [survey_scale.index(res) for res in survey_responses]
    
average_smoking = sum(s)/len(survey_responses)
...
male_acs = [savings[i] for i in range(0,len(gender)) if gender[i]  == 'male']
f_acs = [savings[i] for i in range(0,len(gender)) if gender[i]  == 'female']


def avg(list_var):
    a = sum(list_var)/len(list_var)
    return a    
male_savings = avg(male_acs)
female_savings = avg(f_acs)
...
plt.hist(test_scores_positive)
median2 = numpy.median(test_scores_positive)
mean = numpy.mean(test_scores_positive)
plt.axvline(median2, color="b")
